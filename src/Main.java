    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < products.length; i++) {
            if (counts[i] != 0) {
                ...
                int intDoBonus = doBonus ? counts[i] + 1 : counts[i];
                int intCounts = (counts[i] / 3 * 2 + counts[i] % 3);
                if (isOnSale) {
                    System.out.println("\t" + products[i] + " " + intDoBonus + " шт. за " + (prices[i] * intCounts) + " руб. (распродажа!)");
                    sum += prices[i] * intCounts;
                } else {
                    System.out.println("\t" + products[i] + " " + intDoBonus + " шт. за " + (prices[i] * counts[i]) + " руб.");
                    sum += prices[i] * counts[i];
                }
            }
        }
